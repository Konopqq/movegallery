...
export default function AdminPage() {
  ...
  const filteredAssets = assets.filter(asset => filter === 'all' || asset.status === filter);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
         <div className="flex flex-col gap-1">
             <div className="text-gray-500 text-sm font-mono">
                Total: <strong className="text-white">{assets.length}</strong> | Shown: <strong className="text-white">{filteredAssets.length}</strong>
             </div>
             <button onClick={() => setIsAdminManagerOpen(true)} className="flex items-center gap-2 text-xs font-bold text-blue-400 hover:text-blue-300 transition mt-1 w-fit">
                <UserGroupIcon className="h-4 w-4" /> Manage Admins
             </button>
         </div>
         <div className="flex bg-[#111] p-1 rounded-xl border border-white/10 overflow-x-auto">
            {(['all', 'pending', 'approved', 'rejected'] as FilterType[]).map((f) => (
                <button key={f} onClick={() => setFilter(f)} className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all whitespace-nowrap ${filter === f ? 'bg-white text-black shadow-lg' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
                    {f}
                </button>
            ))}
         </div>
                ...
                {editingId === asset.id ? (
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-3 gap-2 mt-auto pt-4 border-t border-white/5">
                            <button onClick={() => handleUpdateStatus(asset.id, 'approved')} className={`flex justify-center bg-[#151515] border border-white/10 rounded-lg py-3 hover:bg-green-600 hover:text-white hover:border-green-500 transition shadow-md ${asset.status === 'approved' ? 'text-green-500 border-green-500/30' : 'text-gray-400'}`} title="Approve"><CheckIcon className="h-5 w-5" /></button>
                            <button onClick={() => handleUpdateStatus(asset.id, 'rejected')} className={`flex justify-center bg-[#151515] border border-white/10 rounded-lg py-3 hover:bg-yellow-600 hover:text-white hover:border-yellow-500 transition shadow-md ${asset.status === 'rejected' ? 'text-yellow-500 border-yellow-500/30' : 'text-gray-400'}`} title="Reject"><XMarkIcon className="h-5 w-5" /></button>
                            <button onClick={() => setItemToDelete({id: asset.id, path: asset.file_path})} className="flex justify-center bg-[#151515] hover:bg-red-600 text-gray-400 hover:text-white border border-white/10 hover:border-red-500 py-3 rounded-lg transition shadow-md" title="Delete"><TrashIcon className="h-5 w-5" /></button>
                        </div>
                    </>
                )}
            </div>
            {actionLoading === asset.id && (
                <div className="absolute inset-0 bg-black/60 backdrop-blur-[1px] flex items-center justify-center z-50 rounded-2xl"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-white"></div></div>
            )}
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-[#111] border border-red-500/30 rounded-2xl w-full max-w-sm p-6 shadow-2xl">
                <div className="flex flex-col items-center text-center gap-4">
                    <div className="p-3 bg-red-500/10 rounded-full border border-red-500/20"><ExclamationTriangleIcon className="h-8 w-8 text-red-500" /></div>
                    <div><h3 className="text-xl font-bold text-white mb-1">Delete Asset?</h3><p className="text-sm text-gray-400">This action cannot be undone.</p></div>
                    <div className="grid grid-cols-2 gap-3 w-full mt-2">
                        <button onClick={() => setItemToDelete(null)} className="py-2.5 rounded-xl text-sm font-bold bg-[#222] text-white hover:bg-[#333] transition">Cancel</button>
                        <button onClick={handleConfirmDelete} className="py-2.5 rounded-xl text-sm font-bold bg-red-600 text-white hover:bg-red-700 shadow-lg shadow-red-900/20 transition">Yes, Delete</button>
                    </div>
                </div>
            </div>
        </div>
